...
class XSampaNVLTI(object):
    ...
    # IOHAVOC: Special phones are tonal or nasalisations that need to be "stuck" or "attached" to their base phone.
    ...
    # These are the reason for the X-SAMPA choice, but conformed to the Kaldi toolkit, built around ARPABET
    ...
    def ipa2xs(self, ipa):
        ...
        # IOHAVOC: Iterate from last element of non-delimited tone sequences looking for special character. Go until
        ...
        return ' '.join(rev_kaldi_xsampa_list)
...
def g2p_epitran(word_list):
    ...
    for w in word_list:
        ...
        # IOHAVOC: attempts to get a correctly delimited XSAMPA list of phonemes.
        # ipa_word_list = epi.trans_list(w)
        # xsampa_word_list = epi.xsampa_list(w)
        # ipa_segs = epi.ft.ipa_segs(epi.transliterate(w))
        # return list(map(self.xsampa.ipa2xs, ipa_segs))
        # ipa_words.append(" ".join(ipa_word_list))
