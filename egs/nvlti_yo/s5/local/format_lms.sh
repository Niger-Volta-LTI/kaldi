...
for lm_suffix in tgsmall tgmed; do
  ...
  # want to compile G.fst for tglarge, as it takes a while. IOHAVOC, possibly no longer necessary
  test=${src_dir}_test_${lm_suffix}
  mkdir -p $test
  cp -r ${src_dir}/* $test
  arpa2fst --disambig-symbol=#0 --read-symbol-table=$test/words.txt $lm_dir/yo_lm_${lm_suffix}.arpa $test/G.fst
